...
public class Combination {
    public ArrayList<Integer[]> newChickenRecipe(int[] stuffArr, int choiceNum) {
        ...
        // combination실행
        result = combination(result, visited, freshStuffArr, choiceNum, new Integer[0], 0);
    ...
    // 사실 리턴타입 void도 상관없음
    private ArrayList<Integer[]> combination(ArrayList<Integer[]> result, boolean[] visited, int[] stuffArr,
        ...
        for (int i = 0; i < stuffArr.length; i++) {
            ...
            if (!visited[i]) {
                ...
                // 재귀 호출
                result = combination(result, Arrays.copyOf(visited, visited.length), stuffArr, choiceNum,
                        copyArr, depth + 1);
                // 방문여부 리셋
//                visited[i] = false;
            }
